...
sptvGLM_adaMetropGibbs2 <- function(y, X, X_tilde, S, time, 
                                    family, n_binom = NULL, 
                                    N.samp, starting, prior,
                                    n.batch = 3, batch.length = 10, 
                                    verbose = TRUE){
  ...
  post_phi_s <- array(dim = N.samp + 1)
  post_phi_t <- array(dim = N.samp + 1)
  post_xi <- array(dim = c(n, N.samp + 1))
  post_beta <- array(dim = c(p, N.samp + 1))
  post_z <- array(dim = c(n*r, N.samp + 1))
  ...
  distS <- dist2mat(distS, 128)
  distT <- dist2mat(distT, 128)
  V_z <- 1/(1 + post_phi_t[1]*distT) * 
    exp(- (post_phi_s[1]*distS) / sqrt(1 + post_phi_t[1]*distT))
  L_z <- chol(V_z)
  ...
  for(s in 1:N.samp){
    ...
    theta_chain <- sptParams_adaMCMC2(n = n, r = r, z = post_z[, s+1], 
                                      theta_starting = c(post_phi_s[s], 
                                                         post_phi_t[s]), 
                                      distS = distS, distT = distT, 
                                      n.batch = n.batch, 
                                      batch.length = batch.length,
...
sptParams_adaMCMC2 <- function(n, r, z, theta_starting, 
                               distS, distT,
                               n.batch, batch.length, n.report,
                               phi_s_Unifa, phi_s_Unifb, 
                               phi_t_Unifa, phi_t_Unifb, nu_z){
  ...
  target <- function(theta){
    phi_s <- logitinv_bounds(theta[1], phi_s_Unifa, phi_s_Unifb)
    phi_t <- logitinv_bounds(theta[2], phi_t_Unifa, phi_t_Unifb)
    
    V_z <- 1/(1 + phi_t*distT) * exp(- (phi_s*distS) / sqrt(1 + phi_t*distT))
    L_z <- chol(V_z)
    Q_z <- lapply(1:r, function(x){ 
      0.5*(nu_z + n)*log(1 + sum(backsolve(L_z, z[((x-1)*n+1):(x*n)], 
                                           transpose = TRUE)^2)/nu_z) })
    
    sumlogdetz <- r * sum(log(diag(L_z)))
    sumlogdens <- do.call('sum', Q_z)
    logPost <- - sumlogdetz - sumlogdens
    logPost <- logPost + log(phi_s - phi_s_Unifa) + log(phi_s_Unifb - phi_s)
    logPost <- logPost + log(phi_t - phi_t_Unifa) + log(phi_t_Unifb - phi_t)
    
    return(logPost)
  }
  
  inits <- array(dim = 2)
  inits[1] <- logit_bounds(theta_starting[1], phi_s_Unifa, phi_s_Unifb)
  inits[2] <- logit_bounds(theta_starting[2], phi_t_Unifa, phi_t_Unifb)
  ...
  post_phi_s <- logitinv_bounds(as.numeric(chain$p.theta.samples[, 1]), 
                                phi_s_Unifa, phi_s_Unifb)
  post_phi_t <- logitinv_bounds(as.numeric(chain$p.theta.samples[, 2]), 
                                phi_t_Unifa, phi_t_Unifb)
