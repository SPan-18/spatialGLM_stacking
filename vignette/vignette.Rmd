...
packages <- c("MASS", "parallel", "progress", "dplyr", "knitr", "CVXR", "Rcpp",
              "ggplot2", "ggpubr", "MBA", "latex2exp",
              "Rfast", "loo", "Rmosek", "geoR", "spBayes")
...
names(post_spParams) <- c("phi_s_1", "phi_s_2", "phi_t_1", "phi_t_2")
spParams_names <- c('phi_s_1' = TeX('$\\phi_{s1}$'), 
                    'phi_s_2' = TeX('$\\phi_{s2}$'),
                    'phi_t_1' = TeX('$\\phi_{t1}$'),
                    'phi_t_2' = TeX('$\\phi_{t2}$'))
spParams_labeller <- as_labeller(spParams_names, default = label_parsed)
...
ggplot(gather(post_spParams), aes(value)) + 
  ...
  geom_density(fill = "skyblue", alpha = 0.25) +
  xlab("") + ylab("Posterior density") +
  facet_wrap(~ key, labeller = spParams_labeller, scales = 'free') +
  theme_bw() + theme(panel.grid = element_blank())
```
