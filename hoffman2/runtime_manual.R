...
stack_fit <- run_time_stack$time
spBayes_time <- run_time_MCMC$time

run_time <- data.frame(n = nseq, # stack_nofit = stack_nofit,
                       stack_fit = stack_fit, 
                       spBayes_time = c(spBayes_time, rep(max(spBayes_time), 4)))

library("tidyverse")
rt <- run_time %>%
  select(n, stack_fit, spBayes_time) %>%
...
ggplot(rt, aes(x = n, y = value)) +
  ...
  labs(color = "Method") +
  scale_color_discrete(labels = c("spBayes", "stacking")) +
  theme_bw() +
